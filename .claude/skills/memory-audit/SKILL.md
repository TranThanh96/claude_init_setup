---
name: memory-audit
description: Audit .claude/memory/ against the actual codebase — find stale claims, dead file references and over-budget files, then propose fixes.
...
## 1. Mechanical checks
Run `python3 scripts/memory-lint.py`. It reports budgets, leftover placeholders, duplicate rule
imports, and an `active.md` that isn't gitignored.

## 2. Verify claims against code
For every entry in `decisions.md` (active ones), `patterns.md`, `troubleshooting.md`, `active.md`,
and the module rules in `.claude/rules/*.md` that have `paths:` frontmatter:
...
- Is `active.md` about work that has already landed?
